...
func PreChecks(configFile *os.File, deviceHost string) error {
	...
	headerInfo := fmt.Sprintf(`============ DEVICE CONFIGURATION ============
Device Hostname: %s
Date: %s
Generated by: %s
`, deviceHost, currentTime, username)

	_, err := fmt.Fprint(configFile, headerInfo)
	if err != nil {
		log.Printf("Failed to write headerInfo to config file for device %s: %v", deviceHost, err)
		return err
	}

	_, err = fmt.Fprintf(configFile, "%s\n", PreChecksHeader)
	if err != nil {
		log.Printf("Failed to write pre-checks headerInfo to config file for device %s: %v", deviceHost, err)
		return err
	}
